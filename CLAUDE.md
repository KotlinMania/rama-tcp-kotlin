# Claude Code Project Instructions — rama-tcp-kotlin

## Project Overview

This is **rama-tcp-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rama-tcp`](https://crates.io/crates/rama-tcp).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rama-tcp/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rama-tcp>
