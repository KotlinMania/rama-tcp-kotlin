...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `RamaTcp.swiftmodule/`
...
//      `import RamaTcp` above would fail at compile time.
//
//   2. The static archive `libRamaTcp.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "RamaTcp" }` and
//      `flattenPackage = "io.github.kotlinmania.ramatcp"` configuration in
...
//      via the `RamaTcpLibrary` product.
//
...
final class RamaTcpExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "RamaTcp swift module imported cleanly")
    }
}
